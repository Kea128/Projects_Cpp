...
class RRTPlanner {
 ...
 private:
  ...
  bool extendSingleStep(const TreeNode* rrtnode, TreeNode*& node,
  ...
  std::string frame_;
  std::map<int, PointCell>
      freespace_;  //在这里，我用一个map来表示栅格地图中没有障碍物的部分，我们可以只在空闲位置随机撒点试试看

  /* rrt elements */
  double threhold_,
      probability_;  //判断到终点的容忍度，以及向终点方向拉扯的撒点概率
