...
RRTPlanner::RRTPlanner()
    : frame_("map"),
      ...
      probability_(0.5) {
  ros::NodeHandle nd;
  map_sub_ = nd.subscribe<nav_msgs::OccupancyGrid>(
      "map", 1, boost::bind(&RRTPlanner::mapCB, this, _1));
  goal_sub_ = nd.subscribe<geometry_msgs::PoseStamped>(
      "/move_base_simple/goal", 1, boost::bind(&RRTPlanner::goalCB, this, _1));
  pose_sub_ = nd.subscribe<geometry_msgs::PoseWithCovarianceStamped>(
      "/initialpose", 1, boost::bind(&RRTPlanner::initialPoseCB, this, _1));
  ...
  for (int i = 0; i < iteration_; i++) {
    if (drawSample()) {
      for (int j = 0; j < current_sampling_.size(); j++) {
        if (extendNearestNode(current_sampling_[j])) return true;
      }
    }
  }
  return false;
...
bool RRTPlanner::extendNearestNode(PointCell random) {
  ...
  for (int i = 0; i < rrt_nodes_.size(); i++) {
    if (hypot(rrt_nodes_[i]->cell.x - random.x,
              rrt_nodes_[i]->cell.y - random.y) <= path_length) {
      path_length = hypot(rrt_nodes_[i]->cell.x - random.x,
                          rrt_nodes_[i]->cell.y - random.y);
      j = i;
    }
  }
  TreeNode* node;
...
bool RRTPlanner::extendSingleStep(const TreeNode* rrtnode, TreeNode*& node,
                                  const PointCell random) {
  double sintheta, costheta;

  sintheta =
      (random.y - rrtnode->cell.y) / sqrt(pow((random.x - rrtnode->cell.x), 2) +
                                          pow((random.y - rrtnode->cell.y), 2));
  costheta =
      (random.x - rrtnode->cell.x) / sqrt(pow((random.x - rrtnode->cell.x), 2) +
                                          pow((random.y - rrtnode->cell.y), 2));
...
bool RRTPlanner::goalReached(const TreeNode* nearestNode) {
  if (sqrt(pow(nearestNode->cell.x - goal_x_, 2) +
           pow(nearestNode->cell.y - goal_y_, 2)) *
          map_resolution_ <
      threhold_)
    return true;
  else
